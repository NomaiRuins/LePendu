...
public class GameManager : MonoBehaviour
{
    ...
    public WebRequest webRequest; // À lier dans l'inspecteur !
    ...
    private void Awake()
    {
        // Vérification pour éviter les erreurs NullReferenceException
        if (webRequest == null)
        {
            Debug.LogError("ERREUR : La référence 'webRequest' n'a pas été assignée dans l'inspecteur du GameManager !");
        }
    }
    ...
    public void StartNewGame()
    {
        ...
        // 50% de chance d'utiliser un mot de la liste locale (si elle n'est pas vide)
        if (wordList.Count > 0 && Random.value > 0.5f)
        {
    ...
    private void InitializeNewGame(string word)
    {
        ...
        {
            Debug.LogError("Impossible de commencer : le mot reçu est vide ! Tentative avec un mot local de secours.");
            if (wordList.Count > 0)
            {
                word = GetRandomWord();
            }
            ...
            {
                Debug.LogError("La liste de secours est aussi vide ! Le jeu ne peut pas commencer.");
                return;
            }
        }
